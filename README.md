...
#### 1.刷新b站临时ck(推荐)
> 未配置使用自己的B站cookie时将会使用临时ck，当旧临时ck也出现风控时，可通过发送 `#刷新b站临时ck`命令进行ck刷新（并执行一次 临时ck(优先)/自定义ck 上报校验），刷新完请等待bot重启刷新状态以应用更改。`风控问题报错频繁：当前或需要每2天执行一次该命令。`
...
"*/15  * * * *" #固定间隔以每15分钟检测一次
"*/31  * * * *" #固定间隔以每31分钟检测一次
"0 5,35 * * * *" #24小时中，每小时固定第5分0秒、第35分0秒检测一次，共2次
"0 5,35,51 * * * *" #24小时中，每小时固定第5分0秒、第35分0秒、第51分0秒检测一次，共3次
```
或其他值
