...
### （一）B站推送 配置
自 2023 年三月起，B站风控升级，为保证旅行者们的使用体验，如果`总是`出现动态推送获取失败，可尝试如下配置一个或多个。风控后执行如下某个配置后，如果仍出现风控，请30min+后再尝试动态推送添加等指令操作，度过风控期。
#### 1.设备挂机B站客户端(推荐)
> 运行bot的设备或局域网内的设备，后台挂机正在使用的登录了自己B站账号的B站app端或web端(浏览器)，可以降低风控概率。
#### 2.增加动态监测间隔时长(推荐)
B站的api风控是一套复杂机制，针对ip而不针对账号,风控的检测区间为10min，风控的持续时常为30min。

> 修改`Yunzai/plugins/trss-xianxin-plugin/config/bilibili.set.yaml`配置项`pushTime`的值为`"*/15  * * * *"`或`"*/30  * * * *"`或其他值，其中15和30代表检测动态的间隔时间(分钟)。

#### 3.配置自定义Cookie(可选)
> 配置使用自己的B站cookie请在 私聊/私信 Bot下发送`#绑定B站ck: xxx`进行添加自己本地浏览器 登录b站 后获取到的B站cookie，其中`xxx`请替换为自己获取到的cookie(为降低风险，仅少量用户使用时可以保留包含的 `SESSDATA=xxxx;`值)。
...
> 4. **风险选项**：如果未绑定自己b站ck而出现总是获取动态失败以及执行 `#刷新b站临时ck` 并挂机一段时间后仍失败，并且在绑定自己的`B站ck`后仍失败的，最后可尝试绑定cookie时添加/保留`SESSDATA=xxxx;`即使用 完整的`个人B站登录ck`（使用 完整的`个人B站登录ck` 不除外封号的可能性，特别是使用该功能的群个数多/推送订阅多的慎用！！）
