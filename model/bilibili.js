...
import { API, BILIBILI_HEADERS, appendUrlQueryParams, getNewTempCk, postExClimbWuzhi, readTempCk, restart, synCookie } from "../util/BiliApi.js";
import { getDmImg, getWbiSign } from "../util/BiliWbi.js";
...
export default class Bilibili extends base {
  ...
  async getBilibiliUserInfo(uid) {
    const UserInfoApiUrl = API.biliSpaceUserInfo;
    let parama = {
      mid: uid,
      jsonp: 'jsonp',
    }
    ...
    const response = await fetch(url, {
      method: "GET",
      headers: lodash.merge(BILIBILI_HEADERS, { 'Host': `api.bilibili.com`, "Cookie": `${cookie}` }),
      redirect: "follow",
    });
    return response;
  }
  ...
  async getBilibiliDynamicInfo(uid) {
    const DynamicApiUrl = API.biliDynamicInfoList;
    let parama = {
      host_mid: uid,
      //timezone_offset: -480,
      //platform: "web",
  ...
  /**通过uid获取up动态数据-拓展操作*/
  async getBilibiliDynamicInfoNew(uid) {
    let setData = xxCfg.getConfig("bilibili", "set");
    const { cookie, mark } = await synCookie();
    ...
    /**是否开启自动刷新tempCk，默认关闭*/
    const tempCkAutoFlash = !!setData.tempCkAutoFlash === true ? true : false;

    (logger ?? Bot.logger)?.mark(`trss-xianxin插件：B站功能当前使用：${mark}`);

    switch (mark) {
      case 'loginCk':
        resData = await this.fetchGetAction(uid, cookie);
        break;
      case 'localCk':
        resData = await this.fetchGetAction(uid, cookie);
        break;
      case 'tempCk':
        ...
        if (tempCkTTL < 3600 * 24 * 28) {
          ...
          if (tempCkAutoFlash == true) {
            resData = await this.tempCkAutoFlashModeFetch(uid, cookie);
  ...
  /**获取动态列表操作*/
  async fetchGetAction(uid, ck) {

    let resData = await this.getBilibiliDynamicInfo(uid);

    if (!resData || resData.code !== 0) {
      (logger ?? Bot.logger)?.mark(`trss-xianxin插件：获取B站动态：遭遇风控，失败`);
      resData = await this.postExCfetchGetAction(uid, ck);
    }
    return resData;
  }

  /**Post校验ExC+获取动态列表数据*/
  async postExCfetchGetAction(uid, ck) {
    let resData;
    const result = await postExClimbWuzhi(ck);
    ...
    if (dataCode !== 0) {
      ...
      resData = await this.getBilibiliDynamicInfo(uid);
    } else if (dataCode === 0) {
  ...
  /**自动刷新tempCK模式：获取动态列表操作*/
  async tempCkAutoFlashModeFetch(uid, ck) {
    const TempCkStatuKey = "Yz:xianxin:bilibili:biliTempCkStatu";
    let resData = await this.getBilibiliDynamicInfo(uid);
    let tempCkStatuTTL = await redis.ttl(TempCkStatuKey);
    let tempCkStatu = parseInt(tempCkStatu) + 1;
    await redis.set(TempCkStatuKey, tempCkStatu, { EX: tempCkStatuTTL });
  ...
  async upTask() {
    ...
    for (let key in pushData) {
      ...
      for (let up of ups) {
        ...
        // 获取 uid 对应的动态信息
        const response = await this.getBilibiliDynamicInfoNew(up.uid);
